...
func withError(fn func(err *C.char) bool) error {
	...
	if fn(&_err[0]) {
		return &SweError{C.GoString(&_err[0]), nil}
	}
...
func setSidMode(mode Ayanamsa, t0, ayanT0 float64) {
...
func planetName(pl Planet) string {
...
func calc(et float64, pl Planet, fl int32) ([]float64, int, error) {
	return _calc(et, fl, func(jd C.double, fl C.int32, xx *C.double, err *C.char) C.int32 {
		return C.swe_calc(jd, C.int(pl), fl, xx, err)
	})
}

func calcUT(ut float64, pl Planet, fl int32) ([]float64, int, error) {
...
func _nodAps(jd float64, pl Planet, fl int32, m NodApsMethod, fn _nodApsFunc) (_, _, _, _ []float64, err error) {
	_jd := C.double(jd)
	_pl := C.int32(pl)
	_fl := C.int32(fl)
	_m := C.int32(m)
...
func nodAps(et float64, pl Planet, fl int32, m NodApsMethod) (nasc, ndsc, peri, aphe []float64, err error) {
...
func nodApsUT(ut float64, pl Planet, fl int32, m NodApsMethod) (nasc, ndsc, peri, aphe []float64, err error) {
...
func getAyanamsaName(ayan Ayanamsa) string {
...
func _houses(lat float64, hsys HSys, fn _housesFunc) (_, _ []float64, err error) {
	_lat := C.double(lat)
	_hsys := C.int(hsys)
...
func housesEx(ut float64, fl int32, lat, lng float64, hsys HSys) ([]float64, []float64, error) {
...
func housesARMC(armc, lat, eps float64, hsys HSys) ([]float64, []float64, error) {
	return _houses(lat, hsys, func(lat C.double, hsys C.int, cusps, ascmc *C.double) C.int {
		_armc := C.double(armc)
		_eps := C.double(eps)
...
func housePos(armc, geolat, eps float64, hsys HSys, pllng, pllat float64) (pos float64, err error) {
	_armc := C.double(armc)
...
func houseName(hsys HSys) string {
